...
{
    // 與建立資料庫有關之變數宣告
    private static String DATABASE_TABLE1 = "tallybook";     //宣告"步數資料表"常數
    private static String DATABASE_TABLE2 = "expenseTypes";  //宣告"運動種類資料表"常數
    ...
    // 與圖形介面元件相關之變數宣告
    private EditText etxtPrice, etxtComment; //分別為"輸入運動步數"與"輸入運動說明"之可編輯文字方塊變數
    ...
    // 其他變數宣告
    String [] initialExpenseTypes = { "慢走", "慢跑", "快走", "快跑"}; //初始消費種類陣列
    ...
    public void btnSave_Click(View view)
    {
        ...
        {
            Toast.makeText(this, "沒有填寫運動步數!", Toast.LENGTH_LONG).show();
            empty++;
        }
        if(ExpenseType.length()==0) // 若沒有選擇消費種類，則利用吐司訊息提醒使用者
        {
            Toast.makeText(this, "選擇運動種類!", Toast.LENGTH_LONG).show();
            empty++;
        }
    ...
    public void btnQuery_Click(View view)
    {
        ...
        {
            tvMainOutput.setText("查詢紀錄時發生例外，原因如下： " + ex.getMessage());
        }
    }
