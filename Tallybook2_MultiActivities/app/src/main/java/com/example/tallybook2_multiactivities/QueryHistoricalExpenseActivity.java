...
public class QueryHistoricalExpenseActivity extends Activity
{
    // 與建立資料庫有關之變數宣告
            ...
            try //利用try{}包住可能會產生例外的處理程式碼(如存取資料庫及檔案)，讓程式即便有例外，也不會當掉，以增加app之強韌性
            {
                if(radioButtonByDate.isChecked()) // 若選擇依照日期先後顯示
                {
                    ...
                    if(cursor != null) // 若有回傳消費紀錄，則進行以下處理
                    {
                        ...
                        String str = "在" + StartDateString + "到" + EndDateString + "共有"+ n + "筆消費紀錄:\n";
                        int totalAmount = 0; // 用於紀錄消費總金額
                        cursor.moveToFirst();  // 將指標移到第1筆紀錄
                        for (int i = 0; i < n; i++) // 利用迴圈逐一讀取每一筆紀錄
                        {
                            totalAmount += Integer.parseInt(cursor.getString(1)); //讀取第1個欄位(即消費金額price)，並加總到消費總金額中
                        ...
                        for (int i = 0; i < n; i++) //利用迴圈讀取每一筆紀錄之各個欄位
                        {
                            ...
                            str += String.format("%8s\t", cursor.getString(1)); // 串接第1個欄位值(即消費金額price)
                            str += String.format("%6s\t", cursor.getString(2)); // 串接第2個欄位值(即消費種類expenseType)
                            str += String.format("%8s\t", cursor.getString(3)); // 串接第3個欄位值(即消費說明comment)
                            str += String.format("%14s\t", cursor.getString(4));// 串接第4個欄位值(即消費日期payDate)
                            str += "\n";
                            cursor.moveToNext();  // 移動到下一筆
                        }
                    ...
                    {
                        tvOutput.setText("在" + StartDateString + "到" + EndDateString + "並沒有消費紀錄!\n");
                    }
                }
                if(radioButtonByType.isChecked()) // 若選擇依消費種類顯示
                {
                    ...
                    {
                        String str1 = "在" + StartDateString + "到" + EndDateString + "之消費統計如下:\n";
                        String str="";
    ...
    public void btnDelete_Click(View v)
    {
        ...
        if (empty==0) // 若使用者有選擇起始日期及結束日期，則進行以下處理
        {
            try //利用try{}包住可能會產生例外的處理程式碼(如存取資料庫及檔案)，讓程式即便有例外，也不會當掉，以增加app之強韌性
            {
                ...
                str = "確定刪除" + StartDateString + "到" + EndDateString + "間之消費紀錄?";
