...
//************************************************************************************
package com.example.tallybook2_multiactivities;
...
{
    // 與建立資料庫有關之變數宣告
    private static String DATABASE_TABLE1 = "tallybook";     //宣告"記帳簿資料表"常數
    private static String DATABASE_TABLE2 = "expenseTypes";  //宣告"消費種類資料表"常數
    ...
    // 其他變數宣告
    String [] initialExpenseTypes = { "餐飲", "購物", "交通", "娛樂", "醫療", "投資", "保險", "教育", "社交", "住宿"}; //初始消費種類陣列
    String [] expenseTypes; //用來消費種類之陣列(用於建立警示對話方塊之單選單)
    Cursor cursor;
    int n;
    ...
    int Price;            // 儲存消費金額之變數
    String ExpenseType="";   // 儲存消費種類之變數
    String Comment="";       // 儲存消費說明之變數
    String PayDateString=""; // 儲存消費日期之變數
    ...
    public void onCreate(Bundle savedInstanceState)
    {
        ...
        try
        {
            ...
            cursor = db.rawQuery(
                    "select DISTINCT tbl_name from sqlite_master where tbl_name = '" + DATABASE_TABLE1 + "'", null);
            if(cursor != null)
            {
                if(cursor.getCount() == 0)	// 沒有資料表，要建立一個資料表。
                {
                    // 執行SQL建立資料表指令， 建立記帳簿資料表(tallybook)，此資料表有以下5個欄位:
            ...
            cursor = db.rawQuery(
                    "select DISTINCT tbl_name from sqlite_master where tbl_name = '" + DATABASE_TABLE2 + "'", null);
            ...
            if(cursor != null)
            {
                if(cursor.getCount() == 0)	// 沒有資料表，要建立一個資料表。
                {
                    ...
                    // _id(編號): 整數，為主鍵，數值會自動增加
                    // expenseType(消費種類):可變長度Unicode字串(長度最大15)，不可為null(空白)
                    db.execSQL("CREATE TABLE expenseTypes (_id integer primary key autoincrement, expenseType nvarch(15) not null)");
                }
            }
    ...
    public void displayPayDate()
    {
        ...
        PayDateString = year + "-" + (monthOfYear+1) + "-" + dayOfMonth; //以所選日期建立一個格式化的消費日期字串
    ...
    public void btnSave_Click(View view)
    {
        ...
        {
            Toast.makeText(this, "沒有填寫消費金額!", Toast.LENGTH_LONG).show();
            empty++;
        }
        if(ExpenseType.length()==0) // 若沒有選擇消費種類，則利用吐司訊息提醒使用者
        {
            Toast.makeText(this, "消費選擇消費種類!", Toast.LENGTH_LONG).show();
            empty++;
        }
        if (PayDateString.length()==0) // 若沒有選擇消費日期，則利用吐司訊息提醒使用者
        {
            Toast.makeText(this, "沒有選擇消費日期!", Toast.LENGTH_LONG).show();
            empty++;
        }
        if (empty==0) // 若使用者有輸入消費金額、選擇消費種類及選擇消費日期，則進行以下處理
        {
            try //利用try{}包住可能會產生例外的處理程式碼(如存取資料庫及檔案)，讓程式即便有例外，也不會當掉，以增加app之強韌性
            {
                ...
                Toast.makeText(this, "成功儲存新的消費紀錄!", Toast.LENGTH_LONG).show(); //提示使用者已成功儲存新的消費紀錄
    ...
    public void btnQuery_Click(View view)
    {
        ...
        {
            tvMainOutput.setText("查詢歷史消費紀錄時發生例外，原因如下： " + ex.getMessage());
        }
    }
    ...
    public void btnManageExpenseType_Click(View view)
    {
        ...
        {
            tvMainOutput.setText("跳至管理消費種類活動發生例外，原因如下： " + ex.getMessage());
        }
    }
