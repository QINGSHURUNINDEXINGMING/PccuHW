...
using TallybookServiceV2CFDB.Models;

namespace TallybookServiceV2CFDB.Controllers
{
    public class TallybookServiceController : ApiController
    {
        #region
        // 用來儲存記帳簿服務結果資料的內部類別，
        // 每一個欄位變數名稱將是服務回傳JSON物件的Key
        private class TallybookServiceResult
        {
            // 儲存記帳簿服務結果之狀態
            public string Status { get; set; }
            // 儲存記帳簿服務之運算結果
            public string TallybookAPIResult { get; set; }
        }

        // 宣告記帳簿服務結果物件變數：tallybookServiceResultObj，用來儲存記帳簿服務的回傳結果
        TallybookServiceResult tallybookServiceResultObj;

        // 建立雲端記帳簿服務資料庫的物件，可以存取TallybookService的Talltbooks及ExpenseTypes資料表
        TallybookServiceV2Model db = new TallybookServiceV2Model();
        #endregion

        #region
        //==== TallybookServiceController類別的建構子(Constructor)，用於建立初始的消費種類資料表 ===
        // 若是第一次建立該資料表，便會存入10種預設的消費種類供使用者選擇
        public TallybookServiceController()
        {
            try
            {
                // 預設的初始消費種類陣列
                ...
                int rowcount = expenseTypes.Count<ExpenseType>();     // 計算資料表資料筆數
        ...
        {
            // 建立記帳簿服務結果資料物件
            tallybookServiceResultObj = new TallybookServiceResult();
            ...
            try
            {
                ...
                tallybookServiceResultObj.Status = "OK";
                tallybookServiceResultObj.TallybookAPIResult = result;

                // 回記帳簿服務結果資料物件，執行環境會因應Client端的請求(利用application/json或application/xml)
                // 分別自動地將物件序列化(Serialize)成JSON或XML字串
                return Ok(tallybookServiceResultObj);
            }
            catch (Exception ex)
            {
                //===== 由於發生例外，因此僅將例外發生原因存入Status欄位，其餘欄位設為空字串 ====
                tallybookServiceResultObj.Status = "Exception";
                tallybookServiceResultObj.TallybookAPIResult = "刪除消費紀錄時發生例外，原因如下： " + ex.Message;
        ...
        public IHttpActionResult GetTallybooks(string startDate, string endDate, int querymode)
        {
            ...
            if (querymode == 0)  // querymode==0，按照消類日期查詢
            {
                try
                {
                    ...
                    str = "在" + sDate.Date.ToString("yyyy-MM-dd") + "到" + eDate.Date.ToString("yyyy-MM-dd") + "共有" + rowCount + "筆消費紀錄:\n" ;
                    ...
                    //=== 並將記帳簿服務之運算結果存入記帳簿服務結果資料物件對應的欄位 ====
                    tallybookServiceResultObj.Status = "OK";
                    tallybookServiceResultObj.TallybookAPIResult = str;
                ...
                catch (Exception ex)
                {
                    //===== 由於發生例外，因此僅將例外發生原因存入Status欄位，其餘欄位設為空字串 ====
                    tallybookServiceResultObj.Status = "Exception";
                    tallybookServiceResultObj.TallybookAPIResult = "查詢消費紀錄時發生例外，原因如下： " + ex.Message;
            ...
            else if (querymode == 1)  // querymode==1，按照消類種類查詢
            {
                ...
                if (rowCount == 0)  // 若記錄數為0，則回傳沒有消費紀錄之訊息
                {
                    ...
                    //===== 並將記帳簿服務之運算結果存入記帳簿服務結果資料物件對應的欄位 =====
                    tallybookServiceResultObj.Status = "No Record";
                    tallybookServiceResultObj.TallybookAPIResult = str;
                ...
                else // 
                {
                    try
                    {
                        str = "在" + sDate.Date.ToString("yyyy-MM-dd") + "到" + eDate.Date.ToString("yyyy-MM-dd") + "之消費統計如下:\n";
                        foreach (var record in records) // 逐一取出消費資料的每一筆紀錄record
                        {
                            ...
                            str += string.Format("{0}: {1}元\n", record.expenseType, typeSum); //格式化串接該消費種類之總金額
        ...
        public IHttpActionResult GetExpenseTypesString()
        {
            ...
            try
            {
                ...
                foreach (var record in result)
                {
                    ...
                    str += string.Format("{0}", record.expenseType);       // 串接消費種類欄位值
                    str += "\n";
                    i++;
                }
            ...
            catch (Exception ex)
            {
                //===== 由於發生例外，因此僅將例外發生原因存入Status欄位，其餘欄位設為空字串 ====
                tallybookServiceResultObj.Status = "Exception";
                tallybookServiceResultObj.TallybookAPIResult = "取出消費種類字串時發生例外，原因如下： " + ex.Message;
        ...
        public IHttpActionResult PostExpenseType(ExpenseType expenseType)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    tallybookServiceResultObj.Status = "Exist";
                    tallybookServiceResultObj.TallybookAPIResult = str;
                }
            ...
            catch (Exception ex)
            {
                //===== 由於發生例外，因此僅將例外發生原因存入Status欄位，其餘欄位設為空字串 ====
                tallybookServiceResultObj.Status = "Exception";
                tallybookServiceResultObj.TallybookAPIResult = "儲存消費種類時發生例外，原因如下： " + ex.Message;
        ...
        public IHttpActionResult DeleteExpenseType(int no)
        {
            ...
            try
            {
                ...
                if ((no < 1) || (no > count))
                {
                    ...
                    tallybookServiceResultObj.Status = "Out of Range";
                    tallybookServiceResultObj.TallybookAPIResult = str;
                }
                else
                {
                    ...
                    {
                        str += string.Format("{0:d2}  ", (i + 1)); // 串接記錄編號(索引值+1)
                        str += string.Format("{0}", record1.expenseType);       // 串接消費種類欄位值
                        str += "\n";
                        i++;
                    }
            ...
            catch (Exception ex)
            {
                //===== 由於發生例外，因此僅將例外發生原因存入Status欄位，其餘欄位設為空字串 ====
                tallybookServiceResultObj.Status = "Exception";
                tallybookServiceResultObj.TallybookAPIResult = "刪除消費種類時發生例外，原因如下： " + ex.Message;
